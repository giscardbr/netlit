...
public class DynamoDBTables {
    ...
    public void createTableForEntity(final Class<?> entityClass) {
        ...
        final boolean created = createTableIfNotExists(this.dynamoDB, createRequest);
        if (created) {
            log.debug("Table of entity {} created successfully!", entityClass.getSimpleName());
        } else {
            log.debug("Table of entity {} already exists", entityClass.getSimpleName());
        }
    }
